...
Structured Expert Judgment (SEJ) provides a mathematically rigorous framework for quantifying uncertainty when empirical data is insufficient or structurally incomplete. To translate divergent individual expert opinions into a cohesive, probabilistic forecast, this study implements the Classical Model, developed by Roger M. Cooke~\cite{cooke1991experts}. The Classical Model operates as a performance-based linear pooling framework. Rather than assigning equal weight to all experts—a naive approach that assumes uniform proficiency—the Classical Model evaluates and rewards experts empirically based on their performance on a set of calibration or ``seed'' variables. These seed variables represent parameters within the relevant domain for which the true values are known to the researcher but hidden from the expert panel during elicitation.\\\newline
